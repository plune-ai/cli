...
it too, and it now closes the run only after every batch is answered.
...
rather than depending on it, so the default name below reaches Playwright projects only through its
own release.
